...
pub fn day_three(){
    ...
    let rucksacks = partition(populate_rucksacks());
...
fn populate_rucksacks() -> String {
    let mut input = File::open(r"C:\Users\owenh\OneDrive\Documents\Coding\Projects\Advent-of-Code\advent_of_code\src\day_three_input.txt").expect("Can't Open File");
    let mut sacks = String::new();
    input.read_to_string(&mut sacks).expect("Can't Read File");
    return sacks;
}
